...
func TestAdminHandler_AccountCRUD(t *testing.T) {
	h, cookie := setupTestAdmin(t)

	// Create
	createBody := `{"name":"groq","type":"groq","base_url":"https://api.groq.com","api_key":"gsk_test","models":{"chat":["llama-3.3-70b"]},"limits":[{"metric":"rpm","max_value":30,"window_secs":60}]}`
	req := httptest.NewRequest("POST", "/admin/api/accounts", strings.NewReader(createBody))
	...
	if len(accounts) != 1 {
		t.Fatalf("accounts: got %d", len(accounts))
	}
	if accounts[0]["name"] != "groq" {
		t.Errorf("name: %v", accounts[0]["name"])
	}

	// Update
	updateBody := `{"name":"groq-updated","type":"groq","base_url":"https://api.groq.com","api_key":"gsk_new","models":{"chat":["llama-3.3-70b"]},"limits":[]}`
	req = httptest.NewRequest("PUT", "/admin/api/accounts/1", strings.NewReader(updateBody))
	req.AddCookie(cookie)
	...
	// Delete
	req = httptest.NewRequest("DELETE", "/admin/api/accounts/1", nil)
	req.AddCookie(cookie)
	req.SetPathValue("id", "1")
	w = httptest.NewRecorder()
...
func TestAdminHandler_StatsOverview(t *testing.T) {
	...
	h.db.InsertRequestLog(store.RequestLog{
		AccountName: "groq", Model: "m", Endpoint: "openai", Status: "success",
